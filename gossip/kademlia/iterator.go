...
type Iterator struct {
	...
	visited map[gossip.PublicKey]struct{}
	results chan gossip.ID
	buckets [][]gossip.ID
...
func NewIterator(node *gossip.Node, table *Table, opts ...IteratorOption) *Iterator {
	it := &Iterator{
		node:   node,
		table:  table,
		logger: node.Logger(),
...
func (it *Iterator) Find(target gossip.PublicKey) []gossip.ID {
...
func (it *Iterator) init(target gossip.PublicKey) {
	it.results = make(chan gossip.ID, 1)

	it.visited = map[gossip.PublicKey]struct{}{
		it.node.ID().ID: {},
		target:          {},
	}

	it.buckets = make([][]gossip.ID, it.numParallelLookups)
...
func (it *Iterator) processLookupBucket(i int) {
	queue := make(chan gossip.ID, it.numParallelRequestsPerLookup)
	results := make(chan []gossip.ID, it.numParallelRequestsPerLookup)
	pending := 0
...
func (it *Iterator) processLookupRequests(in <-chan gossip.ID, out chan<- []gossip.ID) {
	for id := range in {
		it.lookupRequest(id, out)
	}
}

func (it *Iterator) lookupRequest(id gossip.ID, out chan<- []gossip.ID) {
