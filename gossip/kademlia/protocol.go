...
func (p *Protocol) Find(target gossip.PublicKey, opts ...IteratorOption) []gossip.ID {
...
func (p *Protocol) Discover(opts ...IteratorOption) []gossip.ID {
...
func (p *Protocol) Ack(id gossip.ID) {
	for {
...
func (p *Protocol) Protocol() gossip.Protocol {
...
func (p *Protocol) Bind(node *gossip.Node) error {
	p.node = node
...
// via (*Protocol).Ack.
func (p *Protocol) OnPeerConnected(client *gossip.Client) {
	p.Ack(client.ID())
}
