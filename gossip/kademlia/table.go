...
type Table struct {
	...
	entries [gossip.SizePublicKey * 8]*list.List
	self    gossip.ID
	size    int
}
...
// given ID.
func NewTable(self gossip.ID) *Table {
...
func (t *Table) Self() gossip.ID {
	return t.self
}

// Last returns the last id of the bucket where target resides within.
func (t *Table) Last(target gossip.PublicKey) gossip.ID {
	t.RLock()
	defer t.RUnlock()

	return t.entries[t.getBucketIndex(target)].Back().Value.(gossip.ID)
}

// Bucket returns all entries of a bucket where target reside within.
func (t *Table) Bucket(target gossip.PublicKey) []gossip.ID {
	t.RLock()
	defer t.RUnlock()

	bucket := t.entries[t.getBucketIndex(target)]
	entries := make([]gossip.ID, 0, bucket.Len())

	for e := bucket.Front(); e != nil; e = e.Next() {
		entries = append(entries, e.Value.(gossip.ID))
	}
...
func (t *Table) Update(target gossip.ID) (bool, error) {
	if target.ID == gossip.ZeroPublicKey {
		return false, nil
	}
	...
	for e := bucket.Front(); e != nil; e = e.Next() {
		if e.Value.(gossip.ID).ID == target.ID { // Found the target ID already inside the routing table.
...
func (t *Table) Recorded(target gossip.PublicKey) bool {
	t.RLock()
	defer t.RUnlock()
	...
	for e := bucket.Front(); e != nil; e = e.Next() {
		if e.Value.(gossip.ID).ID == target {
			return true
		}
	}
...
func (t *Table) Delete(target gossip.PublicKey) (gossip.ID, bool) {
	t.Lock()
	defer t.Unlock()
...
func (t *Table) DeleteByAddress(target string) (gossip.ID, bool) {
	t.Lock()
	defer t.Unlock()
...
func (t *Table) FindClosest(target gossip.PublicKey, k int) []gossip.ID {
	var closest []gossip.ID
...
func (t *Table) Entries() []gossip.ID {
	t.RLock()
	defer t.RUnlock()

	entries := make([]gossip.ID, 0, t.size)
...
func (t *Table) getBucketIndex(target gossip.PublicKey) int {
	l := PrefixLen(XOR(target[:], t.self.ID[:]))
	if l == gossip.SizePublicKey*8 {
		return l - 1
	}
